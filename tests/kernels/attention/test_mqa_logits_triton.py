...
def _has_custom_op(name: str) -> bool:
    ...
    try:
        ...
        return hasattr(torch.ops, "_C") and hasattr(torch.ops._C, name)
    except Exception:
        return False
...
def _has_fp8_mqa_logits_cuda() -> bool:
    return _has_custom_op("fp8_mqa_logits_cuda")


def _has_fp8_mqa_logits_cuda_v5() -> bool:
    return _has_custom_op("fp8_mqa_logits_cuda_v5")
...
@pytest.mark.skipif(
    not _has_fp8_mqa_logits_cuda_v7(),
    reason="CUDA fp8 MQA logits v7 custom op is unavailable",
)
@pytest.mark.parametrize(
    "M,N,num_heads,partial_mask",
    [
...
def test_fp8_mqa_logits_cuda_v7_matches_cuda_v5_and_triton(
    M, N, num_heads, partial_mask
):
    torch.manual_seed(0)
    head_dim = 128
    ...
    out_cuda_v7 = fp8_mqa_logits_cuda_v7(
        q_fp8, (k_fp8, k_scales), weights, ks, ke, fallback=False
    )

    inf_triton = torch.isinf(out_triton) & (out_triton < 0)
    assert torch.equal(inf_triton, torch.isinf(out_cuda_v7) & (out_cuda_v7 < 0))
