...
__global__ void sparse_mla_m1_split_final_kernel(
    ...
    float* __restrict__ partial_meta, int64_t q_nope_stride_head,
    int64_t q_nope_stride_d, int64_t q_pe_stride_head,
    int64_t q_pe_stride_d, int64_t kv_stride_token, int64_t kv_stride_d,
    int64_t indices_stride_k, int64_t partial_acc_stride_head,
    int64_t partial_acc_stride_split, int64_t partial_meta_stride_head,
    int64_t partial_meta_stride_split, int topk, int seq_kv, int num_splits,
    float sm_scale) {
  __shared__ float reduce[kNumWarps];
...
#pragma unroll
  ...
  for (int k_pos = split_start; k_pos < split_end; ++k_pos) {
    ...
    const float warp_sum = warp_reduce_sum(qk_part);
    if (lane == 0) {
      reduce[warp_id] = warp_sum;
    }
    __syncthreads();
    if (tid == 0) {
...
#pragma unroll
    ...
    const float qk =
        (kv_idx >= 0 && kv_idx < seq_kv) ? reduce[0] * sm_scale : kNegInf;
    const float n_e_max = fmaxf(e_max, qk);
    const float old_scale = (e_sum > 0.0f) ? __expf(e_max - n_e_max) : 0.0f;
    const float p = (qk == kNegInf) ? 0.0f : __expf(qk - n_e_max);
...
__global__ void sparse_mla_m1_merge_final_kernel(
    const float* __restrict__ partial_acc,
    const float* __restrict__ partial_meta, __nv_bfloat16* __restrict__ out,
...
void check_sparse_mla_m1_splitmerge_inputs(
...
void sparse_mla_m1_splitmerge_final_cuda(const torch::Tensor& q_nope,
                                         ...
                                         int64_t num_splits) {
#ifndef USE_ROCM
  check_sparse_mla_m1_splitmerge_inputs(q_nope, q_pe, kv, indices, partial_acc,
  ...
  const int num_heads = static_cast<int>(q_nope.size(1));
  ...
  sparse_mla_m1_split_final_kernel<<<num_heads * splits, kThreads, 0,
                                     stream>>>(
      q_nope_ptr, q_pe_ptr, kv_ptr, indices_ptr, partial_acc_ptr,
      partial_meta_ptr, q_nope.stride(1), q_nope.stride(2), q_pe.stride(1),
      q_pe.stride(2), kv.stride(0), kv.stride(2), indices.stride(2),
      partial_acc.stride(0), partial_acc.stride(1), partial_meta.stride(0),
      partial_meta.stride(1), topk, seq_kv, splits,
      static_cast<float>(sm_scale));
  check_cuda_launch("sparse_mla_m1_split_final_kernel");

  sparse_mla_m1_merge_final_kernel<<<num_heads * kMergeTiles, kThreads, 0,
                                     stream>>>(
      partial_acc_ptr, partial_meta_ptr, out_ptr, partial_acc.stride(0),
      partial_acc.stride(1), partial_meta.stride(0), partial_meta.stride(1),
      out.stride(1), out.stride(2), splits);
  check_cuda_launch("sparse_mla_m1_merge_final_kernel");
#else
  TORCH_CHECK(false,
              "sparse_mla_m1_splitmerge_final_cuda is not supported on ROCm");
#endif
}
